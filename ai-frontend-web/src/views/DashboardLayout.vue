...
const routeToTabKey: Record<string, string> = {
  ...
  "/backtest/results": "backtest-strategy",
...
// 处理导航项点击
const handleNavItemClick = (routePath: string) => {
  const item = routeToTabKey[routePath];
  if (!item || !navItemToTab[item]) {
...
// 添加调试日志
console.log(
  "🔥 DashboardLayout: handleViewStrategy function defined:",
  !!handleViewStrategy,
);
...
const handleBackToList = () => {
  if (tabSystemRef.value?.hasTab("strategy-create")) {
    tabSystemRef.value.closeTab("strategy-create");
  }
...
const goToBacktest = () => {
  handleNavItemClick("strategy-optimization");
};
