...
public class UrlParamsBuilder {
  ...
  private final List postBodyList = new ArrayList();
  ...
  public UrlParamsBuilder putToList(Object obj) {
    postBodyList.add(obj);
    return this;
  }
  ...
  public RequestBody buildPostBody() {
    if (postBodyMap.stringListMap.isEmpty() && postBodyList.isEmpty()) {
    ...
    } else if (!postBodyList.isEmpty()) {
      return RequestBody.create(JSON_TYPE, JSON.toJSONString(postBodyList));
    }else {
      HashMap<Object, Object> paramMap = new HashMap<>();
      paramMap.putAll(postBodyMap.stringListMap);
      paramMap.putAll(postBodyMap.map);
      return RequestBody.create(JSON_TYPE, JSON.toJSONString(paramMap));
    }
  }
