  ...
  <div class="quick-trade-page">
    <div class="watchlist-bar">
      <div class="watchlist-scroll">
        ...
        >{{ item.name || shortSymbol(item.symbol) }}</div>
    ...
    <div class="chart-wrap">
      <KlineChart
        ...
        :sym-name="selectedName"
        :symbol="form.symbol || defaultKlineSymbol"
        :height="350"
        noPadding
        :credential-label="selectedCredentialLabel"
        :balance-available="balance?.available"
        :balance-total="balance?.total"
        :balance-currency="balance?.currency"
        @credential-click="openCredentialPicker"
        @refresh-balance="refreshTradeData"
      />
    </div>
    ...
    <div class="panel-card">
      ...
      <van-cell
        ...
        :value="selectedName || form.symbol || $t('watchlist.tap_to_select')"
        is-link
...
export default {
  ...
  computed: {
    ...
    credentialExchange() {
      ...
      return credential?.exchange_id ? String(credential.exchange_id).toUpperCase() : ''
    },
    ...
    /** 默认 K 线交易对，类似 PC 端市场行情始终显示 ETH 永续 */
    defaultKlineSymbol() {
  ...
  watch: {
    selectedCredentialId: {
      ...
      handler() {
        ...
        // 凭证切换时同步更新交易所
        this.selectedExchange = this.credentialExchange
      }
    },
    marketType() {
      this.refreshTradeData()
  ...
  methods: {
    async bootstrap() {
      try {
        ...
        if (!this.form.symbol) {
          const defaultSymbol = this.watchlistStore.activeSymbol ||
            (this.watchlistCrypto.length > 0 ? this.watchlistCrypto[0].symbol : 'ETH-USDT-SWAP')
          this.form.symbol = defaultSymbol
          this.watchlistStore.setActive(defaultSymbol, 'Crypto')
        }
    ...
    onPickSymbol(item) {
      ...
      this.selectedName = item.name || item.symbol || ''
      this.selectedExchange = item.exchange || this.credentialExchange
