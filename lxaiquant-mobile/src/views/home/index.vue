  ...
  <div class="home-page">
    ...
    <div class="ios-section">
      ...
      <div class="app-grid">
        ...
        <div class="app-tile" @click="$router.push('/trading')">
    ...
    <div class="ios-section">
      ...
      <div v-if="watchlistItems.length" class="watchlist-strip">
        ...
        >
          <div class="wl-head">
            <span class="wl-sym">{{ item.name || shortSymbol(item.symbol) }}</span>
            <span class="wl-market">{{ item.exchange || 'GATEIO' }}</span>
          </div>
          <span class="wl-market-tag">{{ item.market || 'Crypto' }}</span>
    ...
    <!-- TODO: MacroCalendarPanel 后端接口暂未实现（/api/global-market/*），后续补全后取消注释 -->
    <!-- <MacroCalendarPanel /> -->
...
<script>
import { credentialsApi, dashboardApi, getBaseUrl, strategyApi, tradingBotApi, watchlistApi } from '@/api'
// TODO: 后端 /api/global-market/* 暂未实现，后续补全后恢复 MacroCalendarPanel 导入
...
export default {
  ...
  computed: {
    ...
    strategyCounts() {
      return this.tradingBotStore.statusCounts
    },
    alertStrategies() {
      return this.tradingBotStore.alertBots
    },
    recentTrades() {
  ...
  mounted() {
    this.loadData()
    // TODO: 后端 /api/market/watchlist/prices 暂未实现，后续补全后取消注释
  ...
  methods: {
    async loadData() {
      this.loading = true
      try {
        const [summaryRes, strategyRes, credentialsRes, notificationRes, watchlistRes] = await Promise.allSettled([
          dashboardApi.getSummary(),
          tradingBotApi.getList(),
          credentialsApi.list(),
        ...
        this.tradingBotStore.setBots(strategyRes.status === 'fulfilled' ? (strategyRes.value.data || []) : [])
    ...
    // TODO: 后端 /api/market/watchlist/prices 暂未实现，后续补全 refreshPrices 方法
    async refreshPrices() {
      this.priceMap = {}
    },
