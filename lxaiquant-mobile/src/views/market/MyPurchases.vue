  ...
  <div class="page">
    ...
    <div v-if="items.length" class="list">
      <div v-for="item in items" :key="item.id" class="card">
        <div class="row">
          <span class="title">{{ item.botName || '-' }}</span>
          <span class="price">{{ Number(item.creditsSpent || 0) > 0 ? `${item.creditsSpent}` : $t('market.price_free') }}</span>
        </div>
        <p class="desc">{{ item.botDescription || '-' }}</p>
        <div class="meta">{{ formatTime(item.purchaseTime) }}</div>
        <div class="actions">
...
export default {
  ...
  methods: {
    async load() {
      try {
        const res = await botMarketApi.getMyPurchases()
    ...
    goDetail(item) {
      ...
      this.$router.push(`/market/indicator/${item.listingId}`)
    },
    goCreate(item) {
      if (!item.listingId) return
      this.$router.push({
        ...
        query: {
          source_indicator_id: item.listingId,
          name: item.botName
        }
      })
    },
