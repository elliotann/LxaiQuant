      ...
      <div class="grid">
        <div v-for="item in items" :key="item.id" class="ind-card" @click="openDetail(item)">
          <div class="ind-cover" :style="coverStyle(item)">
            ...
            <div class="cover-top">
              <span class="cover-badge">
                <van-icon :name="item.pricingType === 'paid' ? 'gold-coin-o' : 'gift-o'" />
                {{ item.pricingType === 'paid' ? $t('market.filter_paid') : $t('market.filter_free') }}
              </span>
              <span v-if="item.vipFree" class="cover-vip">VIP</span>
            </div>
          ...
          <div class="ind-body">
            ...
            <div class="ind-stats">
              <span class="stat">
                <van-icon name="star" /> {{ Number(item.avgRating || 0).toFixed(1) }}
              </span>
              <span class="stat">
                <van-icon name="cart-o" /> {{ item.purchaseCount || 0 }}
              </span>
              <span :class="['price', item.pricingType === 'paid' ? 'paid' : 'free']">
                {{ item.pricingType === 'paid' ? $t('market.price_credits', { price: item.price }) : $t('market.price_free') }}
              </span>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async loadMore() {
      ...
      this.loading = true
      try {
        const res = await botMarketApi.getListings({
          page: this.page,
          pageSize: this.pageSize,
          keyword: this.keyword || undefined,
          pricingType: this.pricing || undefined,
          sortBy: this.sort
        })
    ...
    typeLabel(item) {
      return (item.tags && item.tags.length > 0) ? item.tags[0] : 'Bot'
    },
    coverStyle(item) {
      const palettes = [
        ['#667eea', '#764ba2'],
