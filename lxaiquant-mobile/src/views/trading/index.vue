...
import { tradingBotApi } from '@/api'
import { useTradingBotStore } from '@/stores'
...
export default {
  ...
  computed: {
    ...
    strategies() {
      return this.tradingBotStore.bots
    },
    statusTabs() {
      const counts = this.tradingBotStore.statusCounts
      return [
  ...
  activated() {
    // activated 每次从 keep-alive 缓存激活时都会执行
    this.loadStrategies()
  },
  ...
  methods: {
    async loadStrategies() {
      this.loading = true
      try {
        const res = await tradingBotApi.getList()
        const bots = Array.isArray(res.data) ? res.data : []
        this.tradingBotStore.setBots(bots)
      } catch (error) {
    ...
    async deleteStrategy(strategy) {
      ...
      try {
        ...
        await tradingBotApi.delete(strategy.id)
    ...
    async startStrategy(strategy) {
      strategy._loading = true
      try {
        await tradingBotApi.start(strategy.id)
    ...
    async stopStrategy(strategy) {
      try {
        ...
        strategy._loading = true
        await tradingBotApi.stop(strategy.id)
