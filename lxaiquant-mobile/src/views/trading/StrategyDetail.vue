      ...
      <div class="hero">
        <div class="hero-top">
          ...
          <div class="hero-text">
            ...
            <div class="hero-tags">
              <span v-if="strategy.status && !isPurchasedBot" :class="['tag', 'status', strategy.status]">{{ statusText }}</span>
        ...
        <div class="hero-metrics">
          <div class="metric">
            ...
            <span :class="['value', perfMetrics.netProfit >= 0 ? 'profit' : 'loss']">
              {{ formatSigned(perfMetrics.netProfit) }}
            </span>
          </div>
          <div class="metric">
        ...
        <van-tab :title="$t('trading.tab_performance')" name="performance">
          <div class="tab-body">
            <div class="section perf-summary">
              <div class="grid">
                <div class="grid-item">
                  <span class="g-label">累计收益</span>
                  <span :class="['g-value', perfMetrics.netProfit >= 0 ? 'profit' : 'loss']">{{ formatSigned(perfMetrics.netProfit) }}</span>
                </div>
                <div class="grid-item">
...
import { strategyApi, credentialsApi, botMarketApi, orderApi, tradingBotDetailApi } from '@/api'
...
export default {
  ...
  computed: {
    ...
    isPurchasedBot() {
      return String(this.$route.params.id || '').startsWith('purchased_')
    },
    purchasedListingId() {
      const id = this.$route.params.id || ''
      return id.startsWith('purchased_') ? id.replace('purchased_', '') : null
    },
    isBotId() {
      return String(this.$route.params.id || '').startsWith('BOT-')
    },
    tc() {
    ...
    perfMetrics() {
      ...
      const out = {
        netProfit: this.performance?.netProfit ?? this.performance?.total_pnl ?? 0,
        totalReturn: 0,
        annualReturn: 0,
        maxDrawdown: 0,
        sharpe: null,
  ...
  mounted() {
    if (this.isPurchasedBot) {
      this.loadPurchasedBot()
    } else {
      this.loadData()
    }
  },
  ...
  methods: {
    async loadData() {
      this.loading = true
      try {
        const api = this.isBotId ? tradingBotDetailApi : strategyApi
        const [detailRes, credRes] = await Promise.allSettled([
          api.getDetail(this.strategyId),
          credentialsApi.list()
        ])
    ...
    /** 加载已购买机器人的详情 + 实盘表现 */
    async loadPurchasedBot() {
      this.loading = true
      try {
        const [detailRes, perfRes] = await Promise.allSettled([
          botMarketApi.getListingDetail(this.purchasedListingId),
          botMarketApi.getPerformance(this.purchasedListingId)
        ])
        // 组装成和 strategy 兼容的数据结构
        const listing = detailRes.status === 'fulfilled' ? detailRes.value?.data : null
        const perf = perfRes.status === 'fulfilled' ? perfRes.value?.data : {}
        this.strategy = listing
          ? {
              id: this.$route.params.id,
              listingId: listing.id,
              name: listing.name || listing.botName || listing.botNameZh || '-',
              description: listing.description || listing.botDescription || '',
              symbol: listing.symbol || '',
              status: '',
              trading_config: {
                symbol: listing.symbol || '',
                timeframe: listing.timeframe || '',
                initial_capital: listing.initialCapital || 0
              },
              bot_display: listing.botDisplay || {},
              icon: listing.icon || ''
            }
          : null
        this.performance = perf
        // 如果有 equityCurve 数据也一并设置
        if (Array.isArray(perf.equity_curve)) {
          this.equityCurve = perf.equity_curve
        }
        // 加载持仓订单
        await this.loadPositions()
      } catch (error) {
        console.error('Load purchased bot detail failed:', error)
        showToast({ message: this.$t('common.error_network') || 'Error', type: 'fail' })
      } finally {
        this.loading = false
      }
    ...
    async loadPositions() {
      try {
        const robotId = this.isPurchasedBot ? this.purchasedListingId : this.strategyId
        const res = await orderApi.list({ robotId, status: 'DEAL' })
        const raw = Array.isArray(res?.data) ? res.data : []
        // 将 OrderVO 字段映射为模板期望的字段名
        this.positions = raw.map(item => ({
          ...item,
          symbol: item.symbol,
          side: item.orderSide || item.side,
          size: item.amount,
          position_size: item.amount,
          entry_price: item.buyAvgPrice || item.openPrice || item.buyPrice,
          avg_entry_price: item.buyAvgPrice || item.openPrice || item.buyPrice,
          mark_price: item.sellPrice || item.openPrice || 0,
          current_price: item.sellPrice || item.openPrice || 0,
          unrealized_pnl: item.income || item.hasIncome || 0,
          pnl: item.income || item.hasIncome || 0
        }))
      } catch {
        this.positions = []
      }
    ...
    async loadTrades() {
      try {
        const api = this.isBotId ? tradingBotDetailApi : strategyApi
        const res = await api.getTrades(this.strategyId, 30)
        const raw = Array.isArray(res?.data) ? res.data : []
        // 将 OrderVO 字段映射为模板期望的字段名
        this.trades = raw.map(item => ({
          ...item,
          id: item.id || item.orderSn,
          type: item.type || item.orderSide,
          symbol: item.symbol || '',
          price: item.buyPrice || item.sellPrice || item.openPrice,
          amount: item.amount || item.volume,
          value: item.orderAmount,
          profit: item.income || item.hasIncome,
          commission: item.charge,
          commission_ccy: item.symbol?.includes('/') ? item.symbol.split('/')[0] : 'USDT',
          created_at: item.orderTime || item.createTime || item.buyTime,
          time: item.orderTime || item.createTime || item.buyTime
        }))
      } catch {
        this.trades = []
      }
    ...
    async loadPerformance() {
      try {
        ...
        const [perfRes, equityRes] = await Promise.allSettled([
          api.getPerformance(this.strategyId),
          api.getEquityCurve(this.strategyId)
        ])
        let curve = []
        if (perfRes.status === 'fulfilled' && perfRes.value?.data) {
          const d = perfRes.value.data
          // 机器人走报表 API，包含 netProfit/totalIncome/totalCharge
          // 策略走原有 performance API
          this.performance = {
            ...this.performance,
    ...
    async loadLogs() {
      try {
        const api = this.isBotId ? tradingBotDetailApi : strategyApi
        const res = await api.getLogs(this.strategyId, 100)
    ...
    async startStrategy() {
      ...
      try {
        await api.start(this.strategyId)
    ...
    async stopStrategy() {
      ...
      this.actionLoading = true
      try {
        await api.stop(this.strategyId)
    ...
    async handleDelete() {
      ...
      const api = this.isBotId ? tradingBotDetailApi : strategyApi
      try {
        await api.delete(this.strategyId)
