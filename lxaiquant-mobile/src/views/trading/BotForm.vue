<template>
  <div class="page" :class="{ 'is-readonly': isReadonly }">
    ...
    <div class="section" ref="sectionBase">
      ...
      <van-cell-group inset>
        ...
        />
        <van-cell :title="$t('bot_create.exchange_account')" :value="credentialLabel" is-link @click="openCredentialPicker" class="credential-field" />
        <van-cell
...
export default {
  ...
  computed: {
    ...
    filteredCredentials() {
      if (!this.isEditMode || !this.botExchangeId) return this.credentials
    ...
    /** 当前用户是否为机器人创建者（可编辑） */
    isOwnBot() {
      if (!this.isEditMode) return true // 新建机器人，可编辑
    ...
    /** 是否为只读模式（购买的机器人） */
    isReadonly() {
      return this.isEditMode && !this.isOwnBot
    }
  },
  watch: {
  ...
  methods: {
    ...
    async hydrateFromEditQuery() {
      ...
      try {
        ...
        this.botExchangeId = ec.exchange_id || '' // 记录交易所用于过滤账户
    ...
    openCredentialPicker() {
      if (!this.filteredCredentials.length) {
        if (this.isEditMode && this.botExchangeId) {
...
.readonly-banner .van-icon { font-size: 16px; }
.page.is-readonly .segmented,
.page.is-readonly .section {
  position: relative;
}
</style>
...
/* 逐元素灰选 */
.page.is-readonly .segmented,
.page.is-readonly .van-cell {
  opacity: 0.65 !important;
}
/* 投入本金保持可编辑 */
.page.is-readonly .capital-field {
  opacity: 1 !important;
  pointer-events: auto !important;
}
/* 账户选择在只读模式也保持可编辑 */
.page.is-readonly .credential-field {
  opacity: 1 !important;
  pointer-events: auto !important;
}
</style>
