...
import { membershipApi, paymentApi, userApi } from '@/api'
...
export default {
  ...
  methods: {
    ...
    async load() {
      this.loading = true
      try {
        const [profileRes, pkgRes, logRes] = await Promise.allSettled([
          userApi.getProfile(),
        ...
        if (pkgRes.status === 'fulfilled' && Array.isArray(pkgRes.value?.data)) {
          const plans = {}
          pkgRes.value.data.forEach(pkg => {
            plans['pkg_' + pkg.id] = {
              name: pkg.name,
              price_usd: pkg.priceUsdt,
              credits: pkg.credits + (pkg.bonusCredits || 0),
              bonus_credits: pkg.bonusCredits || 0,
              popular: false,
              description: pkg.bonusCredits ? `赠送${pkg.bonusCredits}积分` : pkg.name
            }
          })
          this.plans = plans
        }
    ...
    async handlePurchase(plan) {
      const packageId = Number(plan.key.replace('pkg_', ''))
      if (!packageId) return
      this.creatingPayment = true
      try {
        const res = await paymentApi.createCreditsPayment({ packageId })
        if (res?.success && res?.paymentId) {
          this.order = {
            order_id: res.paymentId,
            payment_id: res.paymentId,
            address: res.paymentAddress,
            amount_usdt: res.amountUsdt,
            status: 'pending',
            payment_uri: res.paymentUri || '',
            expires_at: res.expiresAt,
            credits: res.credits,
            bonus_credits: res.bonusCredits
          }
          this.payVisible = true
          await this.generateQr()
          this.startPolling()
        } else {
          showToast({ message: res?.message || this.$t('profile.pay_create_fail'), type: 'fail' })
        }
      } catch (err) {
        const msg = err?.response?.data?.message || this.$t('profile.pay_create_fail')
    ...
    async refreshOrder(isPolling = false) {
      if (!this.order?.payment_id) return
      if (!isPolling) this.refreshing = true
      try {
        const res = await paymentApi.getPaymentById(this.order.payment_id)
        if (res?.success && res?.payment) {
          this.order = { ...this.order, ...res.payment, order_id: res.payment.id }
