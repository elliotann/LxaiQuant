          ...
          <div v-else class="list">
            ...
            >
              ...
              <div class="row-side">
                ...
                <van-tag v-if="item.exchange" plain size="mini" style="margin-left:4px">{{ item.exchange }}</van-tag>
                <van-icon
                  class="del-icon"
                  name="delete-o"
...
export default {
  ...
  methods: {
    ...
    async chooseResult(item) {
      ...
      this.$emit('pick', { market, symbol, name, exchange })
      this.$emit('update:show', false)
    },
    pick(item) {
      this.$emit('pick', item)
    ...
    async handleRemove(item) {
      try {
        await watchlistApi.remove(item.symbolId)
        await this.load()
