...
@Slf4j
@Service
public class SmoothSignService extends DefaultSignService {

    private static final String SERVICE_KEY = "RangeFilterDWSignService";

    // 配置参数
    @Value("${strategy.range-filter.sampling-period:100}")
    private int samplingPeriod;      // per

    @Value("${strategy.range-filter.range-multiplier:2.5}")
    private double rangeMultiplier;  // mult

    // 可选：是否启用方向过滤（默认true）
    @Value("${strategy.range-filter.enable-direction-filter:true}")
    private boolean enableDirectionFilter;
    ...
    public BuyAndSellWeightDto execute(IndicatorCalcDto calcDto) {
        ...
        int minRequiredBars = samplingPeriod * 2 + 10; // 确保有足够数据计算EMA
    ...
    /**
     * 计算平滑范围 (smrng)
     * 公式: avrng = EMA(|close - close[1]|, per)
     *       wper = 2 * per - 1
    ...
    private double[] calculateSmoothRange(double[] close, int per, double mult) {
        ...
        // EMA of absDiff with period = per
    ...
    private void applyConfiguredParams() {
        ...
        enableDirectionFilter = getBoolean(params, "enableDirectionFilter", enableDirectionFilter);
    }
    ...
    private void applyOverrideParams(Map<String, String> overrides) {
        ...
        enableDirectionFilter = getBooleanOverride(overrides, "enableDirectionFilter", enableDirectionFilter);
    }
